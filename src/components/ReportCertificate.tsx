...
export const ReportCertificate: React.FC<IReportCertificateProps> = ({
  ...
  return (
    <div className="bg-zinc-950/20 border border-zinc-900 rounded-2xl p-4 sm:p-6 sm:p-8 space-y-4 sm:space-y-6 print-card relative">
      
      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row justify-between items-start border-b border-zinc-900 pb-4 sm:pb-5 gap-4">
        <div>
          <h3 className="text-sm sm:text-base font-bold text-white print-title flex items-center">
            <ClipboardList className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-450 mr-2 shrink-0" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 bg-zinc-950/30 p-3 sm:p-4 rounded-xl border border-zinc-900 print-card">
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <div>
            <p className="text-[7px] sm:text-[8px] uppercase text-zinc-500 tracking-wider font-bold">Farmer</p>
            <h4 className="text-xs sm:text-sm font-bold text-zinc-200">{farmer?.name}</h4>
            <p className="text-[9px] sm:text-[10px] text-zinc-400">{farmer?.phone}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <div>
            <p className="text-[7px] sm:text-[8px] uppercase text-zinc-500 tracking-wider font-bold">Farming Plot</p>
            <h4 className="text-xs sm:text-sm font-bold text-zinc-200">{plot?.plotName}</h4>
            <p className="text-[9px] sm:text-[10px] text-zinc-400">{plot?.areaHectares} Hectare(s) • {plot?.crop}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <div>
            <p className="text-[7px] sm:text-[8px] uppercase text-zinc-500 tracking-wider font-bold">Crop Cycle</p>
            <h4 className="text-xs sm:text-sm font-bold text-zinc-200">Stage: {plot?.cropStage}</h4>
            <p className="text-[9px] sm:text-[10px] text-zinc-400">Planted: {plot?.plantingDate}</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2 sm:space-y-2.5">
        <h4 className="text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-zinc-400 flex items-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          <div className="bg-zinc-950/30 border border-zinc-900 p-3 rounded-xl print-card text-center">
            <span className="text-[7px] sm:text-[8px] text-zinc-500 uppercase tracking-wider block">Soil pH</span>
            <span className="text-xs sm:text-sm font-bold text-zinc-200 mt-1 block">{reading?.ph}</span>
          </div>
          <div className="bg-zinc-950/30 border border-zinc-900 p-3 rounded-xl print-card text-center">
            <span className="text-[7px] sm:text-[8px] text-zinc-500 uppercase tracking-wider block">Nitrogen</span>
            <span className="text-xs sm:text-sm font-bold text-zinc-200 mt-1 block">{reading?.nitrogen} ppm</span>
          </div>
          <div className="bg-zinc-950/30 border border-zinc-900 p-3 rounded-xl print-card text-center">
            <span className="text-[7px] sm:text-[8px] text-zinc-500 uppercase tracking-wider block">Phosphorus</span>
            <span className="text-xs sm:text-sm font-bold text-zinc-200 mt-1 block">{reading?.phosphorus} ppm</span>
          </div>
          <div className="bg-zinc-950/30 border border-zinc-900 p-3 rounded-xl print-card text-center">
            <span className="text-[7px] sm:text-[8px] text-zinc-500 uppercase tracking-wider block">Potassium</span>
            <span className="text-xs sm:text-sm font-bold text-zinc-200 mt-1 block">{reading?.potassium} ppm</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2 sm:space-y-2.5">
        ...
        <div className="space-y-2">
          {recData.recommendations?.map((rec, idx) => (
            <div key={idx} className="bg-zinc-950/20 border border-zinc-900 p-3 sm:p-3.5 rounded-xl print-card space-y-1.5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <span className="text-[8px] sm:text-[9px] font-semibold uppercase tracking-wider text-zinc-500">
                  {rec.parameter === 'ph' ? 'Soil pH' : rec.parameter.toUpperCase()}
                </span>
                <span className="text-[8px] sm:text-[9px] font-bold text-emerald-400 bg-emerald-500/5 border border-emerald-950/30 px-2 py-0.5 rounded-lg whitespace-nowrap">
      ...
      <div className="bg-emerald-950/10 border border-emerald-900/20 rounded-xl p-3 sm:p-4.5 print-card">
        <h4 className="text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-emerald-400 mb-2 sm:mb-2.5">Total commercial fertilizer required</h4>
        <div className="divide-y divide-emerald-900/10 space-y-2">
          {!recData.totalFertilizers || recData.totalFertilizers.length === 0 ? (
            <p className="text-xs sm:text-sm text-zinc-400 pt-1">No commercial fertilizer inputs required for this plot.</p>
          ) : (
